...
Intent i=getIntent();
        comp_name1=i.getStringExtra("CompName");
       if(comp_name1!="" && comp_name1!=null)
        {
        SharedPreferences.Editor editor = preferences.edit();
        editor.putString("comp",comp_name1);
        editor.commit();
        }
        complist=(ListView)findViewById(R.id.noti);
        comp1=new ArrayList<>();
        comp_name=preferences.getString("comp","");
        receive1= FirebaseDatabase.getInstance().getReference("order").child("company").child(comp_name).child("orders");
    }
